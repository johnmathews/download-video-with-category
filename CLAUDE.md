...
- `_yt_fitness_pick_target()` — the questions (shows list → seasons list → new names → order for a new season); answers on stdin, prompts on stderr, prints `Show/Spec` (+ a second line with the order for a new season); `YT_FITNESS_ANSWERS_FROM_STDIN=1` lets tests drive it without a tty
- `_yt_fitness_ask_order()` / `_yt_fitness_season_order()` — the feed/course question and the `--season-order` command. Seasons carry a `.order` marker file (`feed` = newest first, numbered down from 999; `course` = 1..N, default); the resolve script reads/writes it and numbers accordingly
